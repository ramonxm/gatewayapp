...
func (r *InvoiceRepository) Save(invoice *domain.Invoice) error {
	_, err := r.db.Exec("INSERT INTO invoices (id, account_id, amount, status) VALUES ($1, $2, $3, $4)", invoice.ID, invoice.AccountID, invoice.Amount, invoice.Status)
...
func (r *InvoiceRepository) FindByID(id string) (*domain.Invoice, error) {
	...
	err := r.db.QueryRow("SELECT id, account_id, amount, status, description, payment_type, card_last_digits, created_at, updated_at FROM invoices WHERE id = $1", id).Scan(&invoice.ID,
		&invoice.AccountID,
		&invoice.Amount,
		&invoice.Status,
		&invoice.Description,
